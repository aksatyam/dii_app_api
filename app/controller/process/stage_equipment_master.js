...
module.exports={
    ...
    getAllStageEquipment: async(req, res)=>{
        try{
            let stageEquipment = await StageEquipment.find({indu_id:req.params.id}).populate({path: 'indu_id'}).populate({path: 'material_master_id'}).populate({path: 'equipment_master_id'});
            if(!stageEquipment)
    ...
    getOneStageEquipment: async(req, res)=>{
        try{
            let stageEquipment = await StageEquipment.findOne({_id:req.params.id}).populate({path: 'indu_id'}).populate({path: 'material_master_id'}).populate({path: 'equipment_master_id'});
            if(!stageEquipment)
            throw validation.errorFormat('Not Found','No Data Available for Industry',404);
            res.status(200).send({msg:'All Industry Data',data:stageEquipment});
        }
